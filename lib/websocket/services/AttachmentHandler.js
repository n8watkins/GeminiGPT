...
class AttachmentHandler {
   ...
   *
   * GIF layout: "GIF87a"/"GIF89a" (6 bytes) then width (UInt16LE) at offset 6
   * and height (UInt16LE) at offset 8.
   *
   * @param {Buffer} buffer - GIF file buffer
   * @returns {Object} Dimensions {width, height} or {0, 0} if not parseable
   * @private
   */
  ...
  /**
   * Parse WebP dimensions for the three container variants (VP8 lossy,
   * VP8L lossless, VP8X extended). Header: "RIFF"(4) size(4) "WEBP"(4),
   * then a 4-byte chunk FourCC at offset 12.
   *
  ...
  parseWebPDimensions(buffer) {
    try {
      ...
      if (fourCC === 'VP8 ') {
        // Lossy: 3-byte frame tag + start code (9d 01 2a) at offset 23, then
        // 14-bit width and height (little-endian) at offsets 26 and 28.
      ...
      if (fourCC === 'VP8L') {
        // Lossless: signature byte 0x2f at offset 20, then a 32-bit LE field
        // packing (width-1):14 and (height-1):14.
      ...
      if (fourCC === 'VP8X') {
        // Extended: 1 flags byte + 3 reserved, then 24-bit LE (width-1) at
        // offset 24 and (height-1) at offset 27.
  ...
  async validateImageDimensions(base64Data, mimeType) {
    try {
      ...
      if (mimeType === 'image/png') {
        ...
        const dimensions = this.parseGIFDimensions(buffer);
        width = dimensions.width;
        height = dimensions.height;

        if (width === 0 || height === 0) {
          console.warn('⚠️  Could not parse GIF dimensions, rejecting for safety');
          return { valid: false, width, height, error: 'Could not parse GIF dimensions' };
        }
      } else if (mimeType === 'image/webp') {
        const dimensions = this.parseWebPDimensions(buffer);
        width = dimensions.width;
        height = dimensions.height;

        if (width === 0 || height === 0) {
          console.warn('⚠️  Could not parse WebP dimensions, rejecting for safety');
          return { valid: false, width, height, error: 'Could not parse WebP dimensions' };
        }
      }
