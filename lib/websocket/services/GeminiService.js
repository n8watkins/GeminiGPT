...
class GeminiService {
  constructor(genAI, tools, functionHandlers) {
    ...
    // Cache for GoogleGenerativeAI instances (keyed by API key fingerprint)
  ...
  /**
   * Create a fingerprint hash of an API key for caching
   * Does NOT reveal the actual key
   *
   * @param {string} key - API key to fingerprint
   * @returns {string} Short hash fingerprint
   * @private
   */
  ...
  /**
   * Get or create cached GoogleGenerativeAI instance
   *
  ...
  getOrCreateGenAI(apiKey) {
    ...
    if (this.genAICache.size >= this.cacheMaxSize) {
      ...
      const firstKey = this.genAICache.keys().next().value;
      this.genAICache.delete(firstKey);
