...
class GeminiLogger {
  ...
  /**
   * Sanitize history for storage (remove large binary data, keep full text)
   */
  _sanitizeHistory(history) {
    ...
    return history.map(msg => ({
      ...
      parts: msg.parts?.map(part => ({
        text: part.text || '', // Keep full text - no truncation
        inlineData: part.inlineData ? { mimeType: part.inlineData.mimeType, size: part.inlineData.data?.length || 0 } : undefined,
  ...
  /**
   * Sanitize parts for storage (remove binary data, keep full text)
   */
