 ...
 *
 * Chunk format (the CURRENT turn leads, prior context trails):
 *   User: <current user message>
 *   Assistant: <current assistant response>
 *
 *   --- Earlier in this chat (for context) ---
 *   <windowed prior turn line(s)...>
 *
 * The current turn leads deliberately: the read path takes a ~200-char snippet
 * from the START of the stored content for citations AND for the context block
 * it injects, so the snippet must reflect THIS turn — not the windowed prior
 * turn. (Leading with the window meant a follow-up turn's snippet showed, and
 * injected, the old context while truncating away the turn's actual content.)
 * The embedding still sees the windowed context regardless of order, so
 * follow-ups like "what about the second one?" keep their referent.
 *
 * Budget handling (when over CHUNK_CHAR_BUDGET):
 *   1. Drop the oldest windowed prior-turn lines first (the trailing context).
 *   2. Only if the current turn alone still exceeds budget, truncate it (from
 *      the end, so the turn's leading content survives in the snippet).
...
function buildTurnChunk(message, responseText, chatHistory, options = {}) {
  ...
  // Current turn leads; windowed prior context trails under a marker (and the
  // marker is omitted entirely when there's no window left). See the module
  // comment for why the current turn must come first.
  const WINDOW_MARKER = '--- Earlier in this chat (for context) ---';
  const assemble = (cLines, wLines) =>
    (wLines.length > 0 ? [...cLines, '', WINDOW_MARKER, ...wLines] : cLines).join('\n');

  // 1. Drop the oldest windowed context lines first until under budget.
  while (windowLines.length > 0 && assemble(currentLines, windowLines).length > charBudget) {
    windowLines = windowLines.slice(1);
  }

  let text = assemble(currentLines, windowLines);
  let truncated = false;
