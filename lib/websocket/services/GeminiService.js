...
class GeminiService {
  ...
  async validateApiKeyWithGemini(apiKey) {
    try {
      ...
      console.warn('API key validation error (non-auth):', error.message);
  ...
  getOrCreateGenAI(apiKey) {
    ...
    if (this.genAICache.has(fingerprint)) {
      ...
      console.log('♻️ Using cached GoogleGenerativeAI instance', { fingerprint });
      return this.genAICache.get(fingerprint);
    }

    // Create new instance
    console.log('🆕 Creating new GoogleGenerativeAI instance', { fingerprint });
    ...
    if (this.genAICache.size >= this.cacheMaxSize) {
      ...
      if (lruKey) {
        ...
        console.log('🗑️ Cache full, evicted LRU entry', {
          evictedKey: lruKey,
  ...
  async createModel(modelName = 'gemini-2.5-flash', apiKey = null) {
    ...
    if (apiKey) {
      if (!this.isValidApiKey(apiKey)) {
        console.warn('⚠️ Invalid client API key format, using server fallback', {
        ...
        if (cached && (now - cached.timestamp) < this.validationCacheDuration) {
          ...
          if (!cached.valid) {
            console.warn('⚠️ Previously validated key is invalid, using server fallback', {
              reason: cached.reason,
              keyPreview: this.sanitizeApiKeyForLogging(apiKey)
            });
            genAI = this.genAI;
          } else {
            console.log('✅ Using validated client API key (cached)', {
              keyPreview: this.sanitizeApiKeyForLogging(apiKey)
            });
          ...
          console.log('🔍 Validating client API key with Google...', {
            keyPreview: this.sanitizeApiKeyForLogging(apiKey)
          });
          ...
          if (!validation.valid) {
            console.warn('⚠️ API key validation failed, using server fallback', {
              reason: validation.reason,
              keyPreview: this.sanitizeApiKeyForLogging(apiKey)
            });
            genAI = this.genAI;
