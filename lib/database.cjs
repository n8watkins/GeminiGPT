...
function createTables() {
  ...
  // the users migration above. Keep this in sync with src/lib/database.ts.
  const geminiLogColumns = new Set(db.prepare('PRAGMA table_info(gemini_logs)').all().map((c) => c.name));
