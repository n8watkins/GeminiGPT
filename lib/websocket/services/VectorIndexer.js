...
class VectorIndexer {
   ...
   * @private
   */
  async indexMessages(userId, chatId, userMessage, assistantMessage, chatTitle, apiKey = null) {
    if (!userId) return;

    try {
      // Index both messages asynchronously
      await Promise.all([
        this.addMessage(userId, chatId, userMessage, chatTitle, {}, apiKey).catch(err =>
          console.error('Error indexing user message:', err)
        ),
        this.addMessage(userId, chatId, assistantMessage, chatTitle, {}, apiKey).catch(err =>
  ...
  async indexMessagePair(userId, chatId, message, responseText, chatHistory, apiKey = null) {
    ...
    await this.indexMessages(userId, chatId, userMessage, assistantMessage, chatTitle, apiKey);
  }
}
