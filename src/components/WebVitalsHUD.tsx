...
interface WebVitalEvent extends Event {
  detail: WebVitalEventDetail
}
...
export function WebVitalsHUD() {
  ...
  useEffect(() => {
    ...
    const handleWebVital = (event: Event) => {
      const webVitalEvent = event as WebVitalEvent
      const metric = webVitalEvent.detail
      setVitals((prev) => ({
        ...prev,
        [metric.name]: {
          name: metric.name,
