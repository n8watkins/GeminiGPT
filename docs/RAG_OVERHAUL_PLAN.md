...
| `ed631ae` | **Threshold tuned from data**: `MAX_DISTANCE` 0.85→0.80, chosen via `tests/manual/threshold-tuning.js` (labelled turn-chunk corpus, precision/recall sweep). Hardest true match 0.670, hardest structural false positive 0.685; 0.80 keeps 100% recall and halves the "my X is named Y" false positives. `MMR_LAMBDA` unchanged. |
