...
class MessagePipeline {
  ...
  checkRateLimit(socket, chatId, userId, apiKey = null) {
    ...
    if (apiKey && typeof apiKey === 'string') {
      // Simple hash function (matches GeminiService fingerprinting)
      let hash = 0;
      ...
      apiKeyFingerprint = Math.abs(hash).toString(16).substring(0, 8);
    }
  ...
  async processMessage(socket, data) {
    ...
    try {
      ...
      const { messageParts, enhancedMessage } = await this.attachmentHandler.processAttachments(attachments, message);
