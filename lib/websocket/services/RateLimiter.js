...
class RateLimiter {
  constructor(config = {}) {
    ...
    // Don't let this background timer keep an otherwise-idle process alive
    // (e.g. test runners or graceful shutdown).
    ...
    if (!this.userLimits.has(userId)) {
      ...
      if (this.userLimits.size >= this.MAX_TRACKED_USERS) {
        ...
        if (this.userLimits.size >= this.MAX_TRACKED_USERS) {
          ...
          // Start from Infinity so the oldest entry is always found even when
          // every user was added within the same millisecond (otherwise nothing
          // is evicted and the map can grow past MAX_TRACKED_USERS).
  ...
  /**
   * Check if user can make a request (supports per-API-key limiting)
   *
   ...
   *
   * @param {string} userId - User identifier
   * @param {string} [apiKeyFingerprint] - Optional API key fingerprint for additional rate limiting
  ...
  checkLimit(userId, apiKeyFingerprint = null) {
    // CRITICAL SECURITY: Validate userId to prevent DoS attacks
    ...
    // CRITICAL SECURITY: Prevent memory exhaustion via extremely long user IDs
    // Maximum reasonable length for user IDs (UUID is 36 chars, add buffer for prefixes)
    const MAX_USER_ID_LENGTH = 256;
    if (userId.length > MAX_USER_ID_LENGTH) {
      console.error(`DoS attempt: userId exceeds max length (${userId.length} > ${MAX_USER_ID_LENGTH})`);
      return {
        allowed: false,
        retryAfter: 60000,
        remaining: { minute: 0, hour: 0 },
    ...
    // CRITICAL SECURITY: Validate API key fingerprint length if provided
    if (apiKeyFingerprint && typeof apiKeyFingerprint === 'string' && apiKeyFingerprint.length > MAX_USER_ID_LENGTH) {
      console.error(`DoS attempt: apiKeyFingerprint exceeds max length (${apiKeyFingerprint.length} > ${MAX_USER_ID_LENGTH})`);
      return {
        allowed: false,
        retryAfter: 60000,
        remaining: { minute: 0, hour: 0 },
    ...
    // If API key fingerprint provided, also check rate limit for that key
    let apiKeyData = null;
    if (apiKeyFingerprint) {
      const apiKeyId = `apikey:${apiKeyFingerprint}`;
      this.initializeUser(apiKeyId);
      apiKeyData = this.userLimits.get(apiKeyId);
      this.refillTokens(apiKeyData.minute, this.limits.minute);
      this.refillTokens(apiKeyData.hour, this.limits.hour);
    }

    // CRITICAL FIX: Atomically check and consume in single step
    // Use Math.max to prevent negative tokens (safety net)
    const userCanProceed = userData.minute.tokens >= 1 && userData.hour.tokens >= 1;
    const apiKeyCanProceed = apiKeyData ? (apiKeyData.minute.tokens >= 1 && apiKeyData.hour.tokens >= 1) : true;
    const canProceed = userCanProceed && apiKeyCanProceed;
    ...
    if (canProceed) {
      ...
      // Also consume API key tokens if applicable
      if (apiKeyData) {
        apiKeyData.minute.tokens = Math.max(0, apiKeyData.minute.tokens - 1);
        apiKeyData.hour.tokens = Math.max(0, apiKeyData.hour.tokens - 1);
        apiKeyData.totalRequests += 1;
        apiKeyData.lastRequest = Date.now();
      }
      ...
      // Determine which limit was hit (user or API key)
      if (!userCanProceed) {
      ...
      } else if (apiKeyData && !apiKeyCanProceed) {
        const hasMinuteToken = apiKeyData.minute.tokens >= 1;
        if (!hasMinuteToken) {
          retryAfter = (apiKeyData.minute.lastRefill + this.limits.minute.refillInterval) - Date.now();
          limitType = 'minute (API key)';
        } else {
          retryAfter = (apiKeyData.hour.lastRefill + this.limits.hour.refillInterval) - Date.now();
          limitType = 'hour (API key)';
        }
      } else {
        // Fallback
        retryAfter = 60000;
