...
| `ed631ae`→`49e8cad` | **Relevance tuned from data** (`tests/manual/threshold-tuning.js`, labelled turn-chunk corpus). Found paraphrase recalls (~0.76) and structural "my X is named Y" false positives (~0.69-0.80) OVERLAP → no absolute threshold separates them (the intermediate `MAX_DISTANCE=0.80` didn't, and the live re-probe confirmed). Final: keep `MAX_DISTANCE=0.85` (recall-safe ceiling) + `RELEVANCE_GAP=0.10` (drop vector candidates trailing the closest match by >gap; correct fact is ~always closest, so no recall cost). Keyword-only hits bypass the gap. `MMR_LAMBDA` unchanged. |
