...
class GeminiService {
  constructor(genAI, tools, functionHandlers, geminiApiLogger = null) {
    this.genAI = genAI;
    this.tools = tools;
    this.functionHandlers = functionHandlers;
    this.geminiApiLogger = geminiApiLogger;
    ...
    geminiLogger.info('✅ GeminiService initialized with config:', {
      ...
      databaseLogging: !!geminiApiLogger
    });
  }
  ...
  isValidApiKey(key) {
    ...
    // Minimum length check (classic keys are 39 chars; AQ. keys are longer)
    ...
    // Valid characters check (alphanumeric, underscore, hyphen, dot)
  ...
  async validateApiKeyWithGemini(apiKey) {
    try {
      ...
      const model = testGenAI.getGenerativeModel({ model: 'gemini-3.1-flash-lite' });
   ...
   * Includes server-side API key verification and safety settings
   *
   * @param {string} modelName - Model name (e.g., 'gemini-3.1-flash-lite')
   * @param {string} apiKey - Optional user-provided API key (BYOK)
   * @returns {Promise<Object>} Gemini model instance
   * @private
   */
  async createModel(modelName = 'gemini-3.1-flash-lite', apiKey = null) {
    ...
    // Configure safety settings to reduce false positives on technical content
   ...
   * @param {Object} socket - Socket.IO socket instance
   * @param {string} chatId - Chat identifier
   * @param {Object} metadata - Additional metadata about the response (blocked, error, etc.)
   * @returns {boolean} True if response is valid
   * @private
   */
  validateResponse(response, message, chatHistory, socket, chatId, metadata = {}) {
    // Check if response is empty
    if (response.length === 0) {
      geminiLogger.error('🚨 CRITICAL: Gemini returned empty response!');
      geminiLogger.error('Response metadata:', {
        wasBlocked: metadata.wasBlocked || false,
        hadError: metadata.hadError || false,
        errorMessage: metadata.errorMessage || 'none',
        finishReason: metadata.finishReason || 'unknown',
        chunkCount: metadata.chunkCount || 0
      });
      geminiLogger.error('Request details:', {
        message: message.substring(0, 100),
        historyLength: chatHistory?.length || 0
      });

      // CRITICAL FIX: Provide more accurate error message based on actual issue
      // Note: Safety filter blocks are handled EARLIER in checkSafetyFilters()
      // If we reach here with empty response, it's likely a different issue
      let errorMessage = 'I encountered an issue generating a response. ';

      if (metadata.wasBlocked) {
        errorMessage += 'The content was blocked by safety filters. Please try rephrasing your question.';
      } else if (metadata.hadError) {
        errorMessage += `Error: ${metadata.errorMessage || 'Unknown error occurred'}. Please try again.`;
      } else if (metadata.chunkCount === 0) {
        errorMessage += 'The AI did not generate any content. This might be a temporary API issue. Please try again.';
      } else {
        errorMessage += 'The response was empty for an unknown reason. Please try rephrasing or try again later.';
      }

      geminiLogger.error('Sending error to user:', errorMessage);
  ...
  async sendMessage(socket, chatId, finalHistory, messageParts, originalMessage = '', chatHistory = [], context = {}) {
    ...
    // Extract API key and userId from context (BYOK)
    const { apiKey, userId } = context;

    // Log request to database
    let logId = null;
    if (this.geminiApiLogger) {
      try {
        logId = this.geminiApiLogger.logRequest({
          chatId,
          userId,
          ...
          metadata: {
            ...
            // Key-class tag for usage tracking: shared server key ('pool')
            // vs visitor-provided key ('byok'). Stored in its own column.
            keyClass: apiKey ? 'byok' : 'pool',
            originalMessage
          }
        });
      } catch (error) {
        geminiLogger.error('Failed to log request to database:', error);
      }
    }
    ...
    const model = await this.createModel('gemini-3.1-flash-lite', apiKey);
    ...
    try {
      ...
      if (this.geminiApiLogger && logId) {
        try {
          this.geminiApiLogger.logError(logId, error);
        } catch (logError) {
          geminiLogger.error('Failed to log error to database:', logError);
        }
      }
    ...
    let usageMetadata = null; // Token counts from the API (last chunk wins)
    ...
    try {
      ...
      for await (const chunk of result.stream) {
        ...
        // Track token usage so it lands in gemini_logs.token_count
        ...
        if (chunk.candidates && chunk.candidates[0] && chunk.candidates[0].finishReason) {
          ...
          geminiLogger.debug('Chunk finish reason:', finishReason);
        }

        // Check for function calls
        // Note: chunk.functionCalls is a METHOD, not a property!
      ...
      geminiLogger.debug('Streaming completed', {
        chunkCount,
      ...
      streamError = error;
      geminiLogger.error('Error during streaming:', {
    ...
    if (hasFunctionCalls && functionCalls.length > 0) {
      ...
      if (this.geminiApiLogger && logId) {
        try {
          this.geminiApiLogger.logResponse(logId, {
            text: functionResponse,
            candidates: [{ content: { parts: [{ text: functionResponse }] } }],
            usageMetadata
          }, functionCalls);
        } catch (error) {
          geminiLogger.error('Failed to log response to database:', error);
        }
      }
      ...
      // Validate response with metadata
      const metadata = {
        chunkCount,
        finishReason,
        hadError: !!streamError,
        errorMessage: streamError?.message,
        wasBlocked: false // Safety blocks are handled earlier
      };

      if (!this.validateResponse(fullResponse, originalMessage, chatHistory, socket, chatId, metadata)) {
        // Log empty response as error
        if (this.geminiApiLogger && logId) {
          try {
            this.geminiApiLogger.logError(logId, new Error('Empty response from Gemini'), 0);
          } catch (error) {
            geminiLogger.error('Failed to log empty response to database:', error);
          }
        }
        return { response: '', error: 'Empty response', metadata };
      }
      ...
      if (this.geminiApiLogger && logId) {
        try {
          this.geminiApiLogger.logResponse(logId, {
            text: fullResponse,
            candidates: [{
              content: { parts: [{ text: fullResponse }] },
              finishReason
            }],
            usageMetadata
          }, []);
